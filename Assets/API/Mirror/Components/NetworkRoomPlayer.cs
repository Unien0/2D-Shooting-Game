...
namespace Mirror
{
    ...
    public class NetworkRoomPlayer : NetworkBehaviour
    {
        ...
        void DrawPlayerReadyState()
        {
            ...
            if (readyToBegin)
                GUILayout.Label("èÄîıäÆóπ");
            else
                GUILayout.Label("èÄîıíÜ");

            if (((isServer && index > 0) || isServerOnly) && GUILayout.Button("í«Ç¢èoÇ∑"))
            {
        ...
        void DrawPlayerReadyButton()
        {
            if (NetworkClient.active && isLocalPlayer)
            {
                ...
                if (readyToBegin)
                {
                    if (GUILayout.Button("ÉLÉÉÉìÉZÉã"))
                        CmdChangeReadyState(false);
                }
                else
                {
                    if (GUILayout.Button(" èÄîıäÆóπ"))
